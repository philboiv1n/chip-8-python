...
def test_ld_vx_byte_sets_register(chip):
    chip.op_ld_vx_byte('V5', 0xAB)
    assert chip.regs['V5'] == 0xAB

def test_add_vx_byte_wraps(chip):
    chip.regs['V5'] = 0xF0
    chip.op_add_vx_byte('V5', 0x20)
    assert chip.regs['V5'] == (0xF0 + 0x20) & 0xFF

def test_ld_vx_vy_copies_register(chip):
    chip.regs['V6'] = 0x3C
    chip.op_ld_vx_vy('V7', 'V6')
    assert chip.regs['V7'] == 0x3C

def test_or_and_xor_and_vx_vy_logical_ops(chip):
    chip.regs['V8'] = 0x0F
    chip.regs['V9'] = 0xF0
    # OR
    chip.op_or_vx_vy('V8', 'V9')
    assert chip.regs['V8'] == (0x0F | 0xF0)
    # AND
    chip.regs['V8'] = 0x0F
    chip.op_and_vx_vy('V8', 'V9')
    assert chip.regs['V8'] == (0x0F & 0xF0)
    # XOR
    chip.regs['V8'] = 0x0F
    chip.op_xor_vx_vy('V8', 'V9')
    assert chip.regs['V8'] == (0x0F ^ 0xF0)
...
def test_add_vx_vy_sets_carry_flag(chip):
    ...
    chip.regs['VB'] = 0x01
    chip.regs['VF'] = 0
    chip.op_add_vx_vy('VA', 'VB')
    assert chip.regs['VF'] == 1
    assert chip.regs['VA'] == 0x00
...
def test_sub_vx_vy_no_borrow(chip):
    ...
    chip.regs['V2'] = 0x20
    chip.regs['V3'] = 0x10
    chip.regs['VF'] = 0  # clear flag
    chip.op_sub_vx_vy('V2', 'V3')
    assert chip.regs['V2'] == 0x10
    assert chip.regs['VF'] == 1
...
def test_sub_vx_vy_with_borrow(chip):
    ...
    chip.regs['V4'] = 0x05
    chip.regs['V5'] = 0x10
    chip.regs['VF'] = 1  # set flag to non-zero
    chip.op_sub_vx_vy('V4', 'V5')
    assert chip.regs['V4'] == (0x05 - 0x10) & 0xFF
    assert chip.regs['VF'] == 0
...
def test_subn_vx_vy_no_borrow(chip):
    ...
    chip.regs['V3'] = 0x20
    chip.regs['VF'] = 0
    chip.op_subn_vx_vy('V2', 'V3')
    assert chip.regs['V2'] == 0x10  # 0x20 - 0x10
    assert chip.regs['VF'] == 1
...
def test_jump_v0_addr_wraps(chip):
    ...
    chip.regs['V0'] = 0xF00
    chip.pc = 0x000
    chip.op_jump_v0_addr(0xA00)
    assert chip.pc == (0xA00 + 0xF00) & 0xFFF
...
def test_skp_vx_no_skip_when_key_not_pressed(chip):
    ...
    chip.regs['V4'] = 0x05   # test with key index 5
    chip.pc = 0x200
    chip.keypad[5] = False
    chip.op_skp_vx('V4')
    assert chip.pc == 0x200
...
def test_add_i_vx_wraps_12bit(chip):
    # FX1E: I should wrap around modulo 0x1000 if sum exceeds 12 bits
    chip.I = 0xF00
    chip.regs['V3'] = 0x200
    chip.op_add_i_vx('V3')
    # 0xF00 + 0x200 = 0x1100, & 0xFFF = 0x100
    assert chip.I == 0x100
...
def test_ld_b_vx_stores_bcd(chip):
    ...
    chip.I = 0x300
    chip.regs['V2'] = 254  # 254 -> digits 2, 5, 4
    chip.op_ld_b_vx('V2')
...
def test_ld_i_vx_stores_registers_in_memory(chip):
    ...
    chip.I = 0x400
    chip.regs['V0'], chip.regs['V1'], chip.regs['V2'], chip.regs['V3'] = 1, 2, 3, 4
    chip.op_ld_i_vx(3)
...
def test_ld_vx_i_loads_memory_into_registers(chip):
    ...
    chip.mem[0x502] = 7
    chip.op_ld_vx_i(2)
    assert chip.regs['V0'] == 9
    assert chip.regs['V1'] == 8
    assert chip.regs['V2'] == 7